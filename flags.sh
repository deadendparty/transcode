...
make_overlay_filter_flags() {
    ...
    local video_index=$(jq ".video.input_index" "$STATE")
...
make_image_sub_flags() {
    ...
    local index 
    while IFS= read -r index; do
        local size=$(get_stream_size "$media" "$index")

        if [[ $size -gt $heaviest_stream_size ]]; then
            local heaviest_stream_index="$index"
            local heaviest_stream_size="$size"
        fi
    done <<< "$indexes"
...
make_subtitle_flags() {
    ...
    local sub_groups=$(group_subs_by_format "$media")

    local text_sub_indexes=$(jq -r ".text[]" <<<"$sub_groups")
    local text_sub_flags=$(make_text_sub_flags "$media" "$text_sub_indexes")
    ...
    local image_sub_indexes=$(jq -r ".image[]" <<<"$sub_groups")
    local image_sub_flags=$(make_image_sub_flags "$media" "$image_sub_indexes")
